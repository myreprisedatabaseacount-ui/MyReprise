...
import { Store, Package, Users, TrendingUp, Settings, Star, ShoppingBag, ChevronDown, BarChart3, Info, Eye, Heart, MessageCircle, ChevronLeft, ChevronRight } from 'lucide-react';
...
const StorePage: React.FC<StorePageProps> = () => {
  ...
  // Récupérer les offres du vendeur avec pagination
  const { data: offersData, isLoading: offersLoading, error: offersError } = useGetOffersBySellerQuery({
    sellerId: userId,
    page: currentPage,
    limit: 10
  }, {
  ...
  const currentPageBackend = pagination.currentPage;
  const limit = pagination.limit;



  // Synchroniser la page actuelle avec le backend
  useEffect(() => {
    if (currentPageBackend && currentPageBackend !== currentPage) {
      setCurrentPage(currentPageBackend);
    }
  }, [currentPageBackend, currentPage]);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="relative h-64 overflow-hidden" style={{
        background: store?.banner 
          ...
          : `linear-gradient(135deg, ${store?.primaryColor || '#4169e1'} 0%, ${store?.secondaryColor || '#ffa500'} 100%)`
      }}>
            ...
            <div className="flex items-end gap-3 sm:gap-4 flex-1">
              ...
              <div className="w-16 h-16 sm:w-20 sm:h-20 bg-white rounded-lg shadow-lg flex items-center justify-center flex-shrink-0">
                {store?.logo ? (
                  ...
                  <Store className="w-8 h-8 sm:w-10 sm:h-10" style={{ color: store?.primaryColor || '#4169e1' }} />
                )}
              </div>

              {/* Store Details */}
              <div className="text-white flex-1 min-w-0">
                <h1 className="text-xl sm:text-2xl font-bold mb-1 truncate">{store?.name || 'Mon Store'}</h1>
                <p className="text-blue-100 mb-2 text-sm sm:text-base line-clamp-2">{store?.description || 'Description du store'}</p>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star
                        key={star}
                        className={`w-3 h-3 sm:w-4 sm:h-4 ${star <= Math.floor(stats.averageRating || 4.8)
                  ...
                  <span className="text-xs sm:text-sm text-blue-100">({stats.averageRating || 4.8})</span>
                  <span className="text-xs sm:text-sm text-blue-200">{stats.totalReviews || 0} avis</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button 
                    className="text-white px-2 sm:px-4 py-2 rounded-lg transition-colors flex items-center gap-1 sm:gap-2 text-sm sm:text-base"
                    style={{ 
                      backgroundColor: store?.primaryColor || '#4169e1'
                    }}
                    onMouseEnter={(e) => {
                      (e.target as HTMLButtonElement).style.backgroundColor = store?.secondaryColor || '#ffa500';
                    }}
                    onMouseLeave={(e) => {
                      (e.target as HTMLButtonElement).style.backgroundColor = store?.primaryColor || '#4169e1';
                    }}
                  >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-6 mb-6 sm:mb-8">
          <div className="bg-white rounded-lg p-4 sm:p-6 shadow-sm border border-gray-200">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-lg sm:text-2xl font-bold text-gray-900">{stats.totalOffers || 0}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg p-4 sm:p-6 shadow-sm border border-gray-200">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="min-w-0">
                <p className="text-xs sm:text-sm text-gray-500">Visiteurs</p>
                <p className="text-lg sm:text-2xl font-bold text-gray-900">{stats.totalViews || 0}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg p-4 sm:p-6 shadow-sm border border-gray-200">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-lg sm:text-2xl font-bold text-gray-900">{stats.completionRate || 0}%</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-4">
              ...
              {totalPages > 1 && (
                <span className="text-sm text-gray-500">
                  Page {currentPageBackend} sur {totalPages}
                </span>
              )}
            </div>
          </div>
          ...
          {offersLoading ? (
            ...
            <>
              {/* Grille responsive : 2 sur mobile, 3 sur tablette, 5 sur PC */}
                  ...
                  >
                    ...
                    <div className="aspect-square bg-gray-100 relative">
                      ...
                      <div className="absolute top-2 right-2">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${offer.status === 'available'
                    ...
                    <div className="p-3">
                      <h4 className="font-medium text-gray-900 mb-1 text-sm line-clamp-2">{offer.title}</h4>
                      <p 
                        className="text-lg font-bold mb-2"
                        style={{ color: store?.primaryColor || '#4169e1' }}
                      >
                        {offer.price} DH
                      </p>
              ...
              {totalPages > 1 && (
                <div className="mt-8 flex items-center justify-center">
                  <div className="flex items-center gap-2">
                    {/* Bouton Précédent */}
                    <button
                      onClick={() => handlePageChange(currentPageBackend - 1)}
                      disabled={currentPageBackend === 1}
                    ...
                    <div className="flex items-center gap-1">
                      {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                        <button
                          ...
                          className={`px-3 py-2 text-sm font-medium rounded-lg ${page === currentPageBackend
                            ? 'text-white'
                            : 'text-gray-500 bg-white border border-gray-300 hover:bg-gray-50'
                            }`}
                          style={page === currentPageBackend ? {
                            backgroundColor: store?.primaryColor || '#4169e1'
                          } : {}}
                        >
                          {page}
                    ...
                    <button
                      onClick={() => handlePageChange(currentPageBackend + 1)}
                      disabled={currentPageBackend === totalPages}
                      className="flex items-center gap-1 px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Suivant
                ...
                <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${selectedOffer.status === 'available'
                        ? 'bg-green-100 text-green-800'
                        : 'bg-gray-100 text-gray-800'
                        }`}>
