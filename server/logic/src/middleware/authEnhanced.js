...
const authenticateToken = async (req, res, next) => {
    try {
        ...
        if (req.cookies && req.cookies.tk) {
            ...
            logger.debug('Token récupéré depuis le cookie');
        }
        ...
        // 2. Si pas de cookie, essayer depuis le header Authorization
            ...
            logger.debug('Token récupéré depuis le header Authorization');
        }
        
        // 3. Si pas de header Authorization, essayer depuis le header 'token'
        else if (req.headers.token) {
            token = req.headers.token;
            logger.debug('Token récupéré depuis le header token');
        }
        ...
        if (!user) {
            logger.warn('Utilisateur non trouvé pour le token', { userId: decoded.userId });
            return res.status(401).json({
                success: false,
        ...
        logger.debug('Authentification réussie', { userId: decoded.userId });
        next();
        ...
        if (token) {
            try {
                ...
                if (user) {
                    ...
                    logger.debug('Authentification optionnelle réussie', { userId: decoded.userId });
                }
            } catch (tokenError) {
